...
export class TaskListComponent {
  ...
  protected getDueDateFormatted(dueDate: string): string {
    ...
    if (totalMinutes < 0) {
      ...
      return `Overdue by ${overdueDays > 0 ? `${overdueDays} day${overdueDays > 1 ? 's' : ''}, ` : ''}${
        overdueHours > 0 ? `${overdueHours} hour${overdueHours > 1 ? 's' : ''}, ` : ''
    ...
    if (days === 0 && hours === 0) {
      ...
      return `In ${days} day${days > 1 ? 's' : ''}, ${hours} hour${hours > 1 ? 's' : ''}, ${minutes} minute${minutes > 1 ? 's' : ''}`;
    }
  }
